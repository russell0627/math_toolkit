...
class _ReflectionViewState extends ConsumerState<ReflectionView> {
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(reflectionCtrlProvider);
    final ctrl = ref.read(reflectionCtrlProvider.notifier);
  ...
  Widget _buildModificationChart() {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(
            "REFLECTION TRANSFORM RULES",
            style: GoogleFonts.shareTechMono(color: Colors.white24, fontSize: 10, fontWeight: FontWeight.bold),
          ),
          const SizedBox(height: 12),
          _buildChartRow("X-AXIS REFLECTION", "(x, y) → (x, -y)"),
          const Divider(height: 12, color: Colors.white10),
          _buildChartRow("Y-AXIS REFLECTION", "(x, y) → (-x, y)"),
          const Divider(height: 12, color: Colors.white10),
          _buildChartRow("ORIGIN REFLECTION", "(x, y) → (-x, -y)"),
        ],
      ),
    );
  }
  ...
  Widget _buildInputSection(ReflectionCtrl ctrl) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
  ...
  Widget _buildResultsSection(ReflectionState state) {
    return Column(
      children: [
        _buildResultCard("X-AXIS REFLECTION", state.x!, -state.y!, Colors.orangeAccent),
        const SizedBox(height: 12),
        _buildResultCard("Y-AXIS REFLECTION", -state.x!, state.y!, Colors.blueAccent),
        const SizedBox(height: 12),
        _buildResultCard("ORIGIN REFLECTION", -state.x!, -state.y!, Colors.purpleAccent),
      ],
    );
  }
  ...
  Widget _buildVisualizer(ReflectionState state) {
    return AspectRatio(
      aspectRatio: 1,
      child: Container(
        decoration: BoxDecoration(
        ...
        child: CustomPaint(
          painter: ReflectionPainter(state),
        ),
      ),
    );
  }
...
class ReflectionPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (state.hasInput) {
      ...
      // X-Reflect (x, -y)
      drawPoint(x, -y, Colors.orangeAccent, "X-REFL");
      // Y-Reflect (-x, y)
      drawPoint(-x, y, Colors.blueAccent, "Y-REFL");
      // Origin Reflect (-x, -y)
      drawPoint(-x, -y, Colors.purpleAccent, "O-REFL");

      // Draw dashed lines to reflections
      ...
      final pO = Offset(center.dx - x * scale, center.dy + y * scale);

      canvas.drawLine(pOrig, pX, dashPaint);
      canvas.drawLine(pOrig, pY, dashPaint);
      canvas.drawLine(pX, pO, dashPaint);
      canvas.drawLine(pY, pO, dashPaint);
    }
  }

  @override
  bool shouldRepaint(covariant ReflectionPainter oldDelegate) => state != oldDelegate.state;
}
